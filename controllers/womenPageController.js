...
const WOMENS_PAGE_TABLE = "womenspage";
...
const getWomensPageData = (req, res) => {
  const response = database.db.query(
    `SELECT * FROM ${WOMENS_PAGE_TABLE}`,
    (err, rows) => {
      if (err) {
...
const addHomeImage = (req, res) => {
  ...
  database.db.query(
    `INSERT INTO ${WOMENS_PAGE_TABLE} (section, subsection, content) VALUES (?,?,?)`,
...
const deleteHomeImage = (req, res) => {
  const { id } = req.params;
  database.db.query(
    `SELECT content FROM ${WOMENS_PAGE_TABLE} WHERE id = ?`,
    [id],
    (err, rows) => {
      if (err) {
      ...
      database.db.query(
        `DELETE FROM ${WOMENS_PAGE_TABLE} WHERE id = ?`,
        [id],
        (err, result) => {
          if (err) {
            return res
...
const deleteRecommendedImage = (req, res) => {
  ...
  database.db.query(
    `SELECT * FROM ${WOMENS_PAGE_TABLE} WHERE id = ?`,
    [id],
    (err, rows) => {
      if (err) {
...
const updateRecommendedImage = (req, res) => {
  ...
  database.db.query(
    ...
    (err, rows) => {
      ...
      database.db.query(
        `UPDATE ${WOMENS_PAGE_TABLE} SET section = ?, subsection = ?, content = ? WHERE id = ?`,
